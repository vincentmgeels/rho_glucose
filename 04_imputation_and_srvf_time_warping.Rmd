...
#ds_cgm_sub1.ordered$bin_tm = cut(ds_cgm_sub1.ordered$device_tm, #breaks=seq(0,86400,by=300),include.lowest=TRUE,labels=seq(1,288,by=1))
#ds_cgm_sub1.ordered$bin_tm <- as.numeric(ds_cgm_sub1.ordered$bin_tm)
#NOTE: to convert from bin_tm (total number of seconds at time of measurement from midnight) to hr:min:sec format, use lubridate::seconds_to_period() OR POSSIBLY chron::times()
...
pt65.day253 <- subset(ds_cgm_sub1,pt_id %in% 65, select = c(pt_id,visitnum,device_dt_tm_days_from_enroll,device_bin_tm,glucose_value))
pt65.day253 <- subset(pt65.day253,device_dt_tm_days_from_enroll %in% 253, select = c(pt_id,visitnum,device_dt_tm_days_from_enroll,device_bin_tm,glucose_value))

plot(pt65.day253$device_bin_tm,pt65.day253$glucose_value,type="l",xlab="time on day=253 from enroll",ylab="glucose value")
...
# # Uses the first day of each patient to create a new "device_day" variable
...
#   mutate(device_day = device_dt_tm_days_from_enroll - device_first_day + 1) %>%
#   mutate(device_day_tm = factor(as.character(device_day)):factor(as.character(device_tm)))
...
pt65.day255 <- subset(ds_cgm_sub1,pt_id %in% 65, select = c(pt_id,visitnum,device_dt_tm_days_from_enroll,device_bin_tm,glucose_value))
pt65.day255 <- subset(pt65.day255,device_dt_tm_days_from_enroll %in% 255, select = c(pt_id,visitnum,device_dt_tm_days_from_enroll,device_bin_tm,glucose_value))

#get glucose values from first patient on day 256 from enroll (id=65) to check
pt65.day256 <- subset(ds_cgm_sub1,pt_id %in% 65, select = c(pt_id,visitnum,device_dt_tm_days_from_enroll,device_bin_tm,glucose_value))
pt65.day256 <- subset(pt65.day256,device_dt_tm_days_from_enroll %in% 256, select = c(pt_id,visitnum,device_dt_tm_days_from_enroll,device_bin_tm,glucose_value))

#plotting selected days against one another
#plot(pt65.day254$device_bin_tm,pt65.day254$glucose_value,type="l",xlab="time",ylab="glucose value")
png("ex_pre_alignment")
plot(pt65.day255$device_bin_tm,pt65.day255$glucose_value,type="l",xlab="time (binned in 5-min increments)",ylab="glucose value",ylim=c(min(pt65.day255$glucose_value,pt65.day256$glucose_value),max(pt65.day255$glucose_value,pt65.day256$glucose_value)),col="red")
points(points(pt65.day256$device_bin_tm,pt65.day256$glucose_value,type="l",xlab="time",ylab="glucose value",col="green"))
...
plot(pt65.day255$device_bin_tm,pt65.day255$glucose_value,type="l",xlab="time (binned in 5-min increments)",ylab="glucose value",ylim=c(min(pt65.day255$glucose_value,pt65.day256$glucose_value),max(pt65.day255$glucose_value,pt65.day256$glucose_value)),col="red")
points(points(pt65.day256$device_bin_tm,pt65.day256align$f2tilde,type="l",xlab="time",ylab="glucose value",col="green"))
...
######################################################################
# bad algorithm for filling in rows corresponding to missing bin_tms #
######################################################################
...
IDs <- unique(ds_cgm_sub1.ordered$pt_id,incomparables = FALSE)
...
#extract a day of interest given the visit (via device_day column)
visitdays <- seq(1,7,by=1)

#in this particular example
visitday <- 2

#extract relevant "baseline" functional data to feed into fdasrvf::pair_align_functions
#returns (# of measurements)x2 tibble
function1 <- ds_cgm_sub1_plus_day %>%
  filter(pt_id == base_id, as.character(visitnum) == as.character(thisvisit), device_day == visitday) %>%
  select(pt_id,trt_group,visitnum,device_day,glucose_value,bin_tm)
...
f1.df %>%
  ggplot(aes(x = bin_tm, y = glucose_value, color = pt_id)) + 
  geom_line()

# data.frame(bin_tm = seq(1, 288, by = 1)) %>% #create 
# full_join(f1.df, by = "bin_tm") %>% 
# mutate(approx = na.approx(glucose_value))

# ex <- f1.df %>% 
#   spread(glucose_value, fill = NA) %>% 
#   gather(glucose_value, dol, -bin_tm)

##########################################################################################################
# ALT APPROACH FOR ADDING MISSING bin_tm cells + NAs - THIS IS HIGHLY INEFFICIENT DUE TO REPEATED rbinds #
##########################################################################################################
...
for (i in 2:length.IDs){
  ...
  function2 <- ds_cgm_sub1_plus_day %>%
    filter(pt_id == IDs[i], as.character(visitnum) == as.character(thisvisit), device_day == visitday) %>%
    select(pt_id,trt_group,visitnum,device_day,glucose_value,bin_tm)
...
#add in missing values/rows
ex <- current.df %>% 
  spread(pt_id, glucose_value, fill = NA) %>% 
  gather(pt_id, glucose_value, -bin_tm)
...
ex %>%
  ...
  ggplot(aes(x = bin_tm, y = glucose_value, color = pt_id)) + 
  geom_line() +
  ggtitle("pt_id = 55, visit 5, day 2: patient curve data before cubic spline interpolation")
ggsave("curve_data_ex_before_interp",device="png")
...
ex %>%
  filter(pt_id == IDs[1]) %>%
  ggplot(aes(x = bin_tm, y = glucose_imputed, color = pt_id)) + 
  geom_line() +
  ggtitle("pt_id = 55, visit 5, day 2: patient curve data after cubic spline interpolation")
ggsave("curve_data_ex_after_interp",device="png")
...
f1.imputed <- ex %>%
  filter(pt_id == IDs[1]) %>%
  select(pt_id,glucose_imputed,bin_tm)
...
for (i in 2:length.IDs){
  #browser()
  #get function2 (glucose levels and bin_tm) from patient with pt_id = IDs[i]
  function2 <- ex %>%
    filter(pt_id == IDs[i]) %>%
    select(pt_id,glucose_imputed,bin_tm)
  
  #align function2 to function1 via srvf framework
  function2_aligned <- fdasrvf::pair_align_functions(f1.imputed$glucose_imputed, function2$glucose_imputed,f1.imputed$bin_tm)
