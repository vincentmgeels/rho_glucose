...
ds_cgm_full %>%
  ...
  ggplot(aes(x = visitnum, y = pt_id, fill = factor(n))) + 
  geom_tile() + 
...
ds_cgm_full %>%
  ...
  labs(title = "Number of Observations within a Time Bin for each Patient in Visit Period 5", 
       fill = "Number of Days")
```
...
To deal with this problem, we decided to average over the glucose measurement observations within a five minute bin. After this was done 
...
```{r eval = FALSE}
ds_cgm_complete %>%
  ggplot(aes(x = device_tm_bin, y = pt_id)) + 
  geom_tile() +
  facet_wrap(visitnum ~ .)

ds_cgm_complete %>%
  filter(visitnum == "5") %>%
  ggplot(aes(x = factor(device_tm_bin), y = pt_id)) + 
  geom_tile()

ds_cgm_sub1 %>%
  filter(pt_id == "222", device_dt_tm_days_from_enroll == "186") %>%
  ggplot(aes(x = device_tm_bin, y = glucose_value)) + 
  geom_point()

ds_cgm_complete %>% 
  count(pt_id, visitnum, device_dt_tm_days_from_enroll) %>%
  ggplot(aes(x = device_dt_tm_days_from_enroll, y = pt_id, fill = n)) + 
  geom_tile() + 
...
levels(as.factor(as.character(ds_cgm_sub1_plus_day$device_day)))
```
...
```{r eval = FALSE}
# Export the complete dataset as both an RDS and csv file
