...
The measurements taken by the CGM were not taken at the same times for all patients. 
...
To deal with this issue, we created time bins for every 5 minutes in a day. Since there are `r 60*60*24` seconds in a day and `r 60*5` seconds in 5 minutes, there are a total of `r 60*60*24/300` time bins that get created. Thus, bin 1 contains observations taken between 00:00:00 to 00:04:59.

- let's figure out how many holes need to be filled in
...
```{r eval = FALSE}
ds_cgm_complete %>%
  filter(visitnum == "5", pt_id == "112") %>%
  count(device_tm_bin)

ds_cgm_complete %>%
  filter(visitnum == "5", pt_id == "112", device_tm_bin == "32") %>% 
  arrange(device_dt_tm_days_from_enroll)
```
...
ds_cgm_sub1_min_time <- ds_cgm_sub1_plus_day %>%
  filter(device_day == "1") %>%
  group_by(pt_id) %>%
  summarise(device_tm_min = min(device_tm)) %>%
...
ds_cgm_sub1_max_time <- ds_cgm_sub1_plus_day %>%
  filter(device_day == "8") %>%
  group_by(pt_id) %>%
  summarise(device_tm_max = max(device_tm)) %>%
