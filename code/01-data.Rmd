...
This document contains the work done to preprocess the data for the Rho group project at IMSM 2019. It includes definitions of the variables in the dataset and visualizations of the data that motivate decisions made when working on the preprocessing. Additionally, the final section contains code for recreating Petra's visualizations of the data to be used in the report.
...
ds_cgm_full <- ds_cgm_full %>%
  ...
  mutate(device_tm_numeric = as.numeric(device_tm),
         device_tm_bin = cut(device_tm_numeric,
           ...
           as.numeric()) %>%
  select(pt_id:device_tm, device_tm_numeric, device_tm_bin, glucose_value)
```
...
ds_cgm_full %>%
  ...
  arrange(desc(n)) %>%
  ggplot(aes(x = device_tm_bin, y = pt_id, fill = n)) + 
  geom_tile() + 
  scale_fill_distiller(palette ="Blues", direction = 1) + 
...
To deal with this problem, we decided to average over the glucose measurement observations within a five minute bin and assign the time of observation to be the midpoint between the times from the multiple observations. The output below shows the top few rows of the number of observations per patient, visit period, and time bin after the glucose values have been averaged it has been averaged. The values were sorted from largest number of observations to smallest number of observations. This shows that the maximum number of observations within a visit period and 5 minute time bin for a patient is 8 (as we want).

```{r}
# Computes average glucose values within a 5 minute time bin
ds_cgm_full_averaged <- ds_cgm_full %>%
  group_by(pt_id, trt_group, pt_status, visit, visitnum, 
           visit_dt_days_from_enroll_lag7, visit_dt_days_from_enroll,
           device_dt_tm_days_from_enroll, device_visit_period_day, 
           device_tm_bin) %>%
  summarise(device_tm_numeric = mean(device_tm_numeric),
            glucose_value = mean(glucose_value)) %>%
...
ds_cgm_full_averaged %>%
  count(pt_id, visitnum, device_tm_bin) %>%
  rename(nobservations = n) %>%
  arrange(desc(nobservations))
```
...
ds_cgm_full %>%
  select(pt_id, 
         ...
         device_tm_numeric, 
         device_tm_bin) %>%
  right_join(ds_cgm_full_averaged %>%
               select(pt_id, 
                      visitnum, 
                      device_visit_period_day, 
                      #device_tm, 
...
### Patients Missing Entire Visit Period(s)
...
complete_participants <- ds_cgm_full_averaged %>%
...
# visit periods
ds_cgm_complete <- ds_cgm_full_averaged %>%
...
### Patients Missing Values Within a Visit Period

**Need to figure out how many holes need to be filled in**
...
Code for Alex to use with her method to create time variables.

```{r}
# Determine the first day for each patient within the visit period
ds_cgm_sub1_pt_first_day <- ds_cgm_sub6 %>%
  group_by(pt_id) %>%
  summarise(device_first_day = min(device_dt_tm_days_from_enroll))

# Uses the first day of each patient to create a new "device_day" variable
# which is the day number when the observation was taken during the visit period
ds_cgm_sub1_plus_day <- ds_cgm_sub6 %>%
  left_join(ds_cgm_sub1_pt_first_day, by = "pt_id") %>%
  mutate(device_day = device_dt_tm_days_from_enroll - device_first_day + 1) %>%
  mutate(device_day_tm = factor(as.character(device_day)):factor(as.character(device_tm_numeric)))

# Save as RDS file
# saveRDS(object = ds_cgm_sub1_pt_first_day,
#         file = "../data/ds_cgm_sub1_plus_day.RDS")

# Determines the earliest time of observation over all patients within a visit period
ds_cgm_sub1_min_time <- ds_cgm_sub1_plus_day %>%
  filter(device_day == "1") %>%
  group_by(pt_id) %>%
  summarise(device_tm_min = min(device_tm_numeric)) %>%
  arrange(device_tm_min) %>%
  slice(1)

# Determines the latest time of observation over all patients within a visit period
ds_cgm_sub1_max_time <- ds_cgm_sub1_plus_day %>%
  filter(device_day == "8") %>%
  group_by(pt_id) %>%
  summarise(device_tm_max = max(device_tm_numeric)) %>%
  arrange(desc(device_tm_max)) %>%
  slice(1)
```
...
The complete data and the 10 subsets of the data were exported as both RDS and csv files.

```{r eval = FALSE}
# Export the complete dataset as both RDS and csv files
