...
### Patients Missing Values Within a Day

- want to select 3 consecutive days 
- determine the 3 consecutive days per person (within a period) with the smallest average missing
...
Work in progress to determine the days to keep....
...
range_with_min_missing_bins <- ds_cgm_complete_rm8_consc3 %>% 
  ...
  spread(key = device_visit_period_day, 
         value = num_missing) %>%
  mutate_at(vars(`1`:`7`), ~replace(., is.na(.), 288)) %>%
  mutate(`1-3` = (`1` + `2` + `3`) / 3,
         `2-4` = (`2` + `3` + `4`) / 3,
         `3-5` = (`3` + `4` + `5`) / 3,
         `4-6` = (`4` + `5` + `6`) / 3,
         `5-7` = (`5` + `6` + `7`) / 3) %>%
  select(-c(`1`:`7`)) %>%
  gather(key = range, value = mean_missing_bins, 3:7) %>%
  group_by(pt_id, visitnum) %>%
  filter(mean_missing_bins == min(mean_missing_bins)) %>%
  group_by(pt_id, visitnum) %>%
...
```{r}
range_with_min_missing_bins %>%
  arrange(desc(mean_missing_bins))

range_with_min_missing_bins %>%
  ggplot(aes(x = mean_missing_bins)) + 
  geom_histogram(bins = 30)
```

```{r}
selected_days <- range_with_min_missing_bins %>%
  select(-mean_missing_bins) %>%
  separate(range, c("1", "3")) %>%
  mutate(`1` = as.numeric(`1`),
         `3` = as.numeric(`3`),
         `2` = (`1` + `3`) / 2) %>%
  select(pt_id:`1`, `2`, `3`) %>%
  gather(key = subrange_day, 
         value = device_visit_period_day, 3:5) %>%
  arrange(pt_id, visitnum)

length(unique(selected_days$pt_id))
length(unique(ds_cgm_subrange$pt_id))
```

```{r}
ds_cgm_subrange <- ds_cgm_complete %>%
  right_join(selected_days, 
             by = c("pt_id", 
                    "visitnum", 
                    "device_visit_period_day")) %>%
  select(pt_id:device_visit_period_day, 
...
```{r}
ds_cgm_subrange %>%
  count(pt_id, visitnum, subrange_day) %>%
  ggplot(aes(x = subrange_day,
             y = pt_id, 
             fill = n)) + 
  geom_tile() + 
  facet_wrap(visitnum ~ .) +
  #scale_fill_distiller(palette ="Blues", direction = 1) + 
  scale_fill_gradientn(limits = c(0,288), colours = c("white", "blue"))

ds_cgm_subrange %>%
  count(pt_id, visitnum, device_tm_bin) %>%
  View()

View(ds_cgm_subrange)
```
