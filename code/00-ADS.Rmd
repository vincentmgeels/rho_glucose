...
dd0 <- readRDS("I:/SHARE/USERS/plebeau/CGM/T1D study data/DataTables/HDeviceCGM.rds") %>% 
  janitor::clean_names() %>% 
...
dd1 <- readRDS("I:/SHARE/USERS/plebeau/CGM/T1D study data/DataTables/HScreening.rds") %>% 
  janitor::clean_names() %>%  
  mutate_if(is.character,as.factor) %>%
  mutate(pt_id=as.factor(pt_id)) %>%
...
dd2 <- readRDS("I:/SHARE/USERS/plebeau/CGM/T1D study data/DataTables/HPtRoster.rds") %>% 
  janitor::clean_names() %>% 
  mutate_if(is.character,as.factor) %>% 
  mutate(pt_id=as.factor(pt_id)) %>%
...
ds_visit <- readRDS("I:/SHARE/USERS/plebeau/CGM/T1D study data/DataTables/HVisitInfo.rds") %>% 
  janitor::clean_names() %>%  
  mutate_if(is.character,as.factor) %>%
  mutate(pt_id=as.factor(pt_id)) %>%
