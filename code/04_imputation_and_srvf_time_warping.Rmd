...
####################################################################################
# first-pass algorithm for filling in rows corresponding to missing device_tm_bins #
####################################################################################

#NOTE: this version of the algorithm currently only handles filling rows in at the day level, not at the visit level!
#considering our subsetted data: ds_cgm_sub1.RDS
...
#############################################################################################################
# APPROACH FOR ADDING MISSING device_tm_bin cells + NAs - THIS IS HIGHLY INEFFICIENT DUE TO REPEATED rbinds #
#############################################################################################################
...
#create new matrix to store patient glucose values (after imputation) for subsequent functional alignment
f.mat <- matrix(NA,ncol=length.IDs,nrow=length.obsns)

#do the cubic spline interpolation and write values to ex$glucose_imputed; also store these values in a matrix as we go for use in subsequent alignment function
for (i in 1:length.IDs){
  #do imputation for patient i
  temp <- ex %>%
    filter(pt_id ==IDs[i]) %>%
    mutate(glucose_imputed = zoo::na.approx(glucose_value)) %>% #requires zoo package
    select(glucose_imputed)
  #write imputed values to ex$glucose_imputed and f.mat
  ex$glucose_imputed[(1+(i-1)*length.obsns):(i*length.obsns)] <- temp$glucose_imputed
  f.mat[,i] <- temp$glucose_imputed #f.mat is of dimension NxM, where N is the number of observed points and M is the number of functions
...
#########################################################################################
#OLD: CUBIC SPLINE APPROACH WE ARE NO LONGER USING. use zoo::na.splines (which performs cubic spline interpolation on missing data)
#create new column to store imputed values
...
#example image for presentation/report
ex %>%
  filter(pt_id == IDs[9]) %>%
  ggplot(aes(x = device_tm_bin, y = glucose_imputed, color = pt_id)) + 
  geom_line() +
  ggtitle("pt_id = 155, visit 5, day 2: patient curve data after linear interpolation")
ggsave("./curve_data_ex_after_lin_interp.png",device="png")


#create new column to store glucose values after alignment
ex$glucose_aligned <- rep(0,288*length.IDs)
...
#store the device_tm_bin information in matrix for use in function alignment package
f.mat.times <- f1.imputed$device_tm_bin

## ALIGNMENT OF CURVES TO ONE ANOTHER USING fdasrvf PACKAGE



#argument f is taken as an NxM matrix of M functions observed at N points (contained in f.mat)
#argument time is taken as an Nx1 vector of N time points (contained in f.mat.times)
f.mat.aligned <- fdasrvf::align_fPCA(f=f.mat, time=f.mat.times)
f.mat.aligned <- fdasrvf::time_warping(f=f.mat,time=f.mat.times,MaxItr=10)

plot(f.mat.times,f.mat.aligned$fmean,type="l",main="mean function after SRSF alignment")

aligned.funs <- as.data.frame(t(f.mat.aligned$fn))

aligned.funs <- aligned.funs %>%
  mutate(pt_id = IDs,aligned.funs)
...
#store baseline reference glucose data (this is unchanged from the original glucose data after imputation)
ex$glucose_aligned[1:288] <- f1.imputed$glucose_imputed
...
for (i in 2:length.IDs){
  ...
  ex$glucose_aligned[(1+(i-1)*length.obsns):(i*length.obsns)] <- function2_aligned$f2tilde
...
###################
ex %>%
  ggplot(aes(x = device_tm_bin, y = glucose_value, color = factor(pt_id))) + 
  geom_line() +
  ggtitle("visit 5, day 2 curve data prior to imputation and srvf alignment")
ggsave("04_ex_curves_before_imputation_and_alignment.png",device="png")

ex %>%
  ggplot(aes(x = device_tm_bin, y = glucose_imputed, color = factor(pt_id))) + 
  geom_line() +
  ggtitle("visit 5, day 2 curve data after linear interpolation and prior to srvf alignment")
ggsave("04_ex_curves_after_lin_interp_before_srvf_alignment.png",device="png")

ex %>%
  ggplot(aes(x = device_tm_bin, y = glucose_aligned, color = factor(pt_id))) + 
  geom_line() +
  ggtitle("visit 5, day 2 curve data after linear interpolation and srvf alignment")
ggsave("04_ex_curves_after_lin_interp_and_srvf_alignment.png",device="png")



```

```{r visitlevelalgorithm}
####################################################################################
# VISIT-level algorithm for filling in rows corresponding to missing device_tm_bins #
####################################################################################

#NOTE: this version of the algorithm handles filling rows in at the visit level
#considering our subsetted data: ds_cgm_sub1.RDS
...
#returns (# of measurements)x2 tibble
function1 <- ds_cgm_sub1 %>%
  filter(pt_id == base_id, as.character(visitnum) == as.character(thisvisit)) %>%
  select(pt_id,trt_group,visitnum,device_visit_period_day,glucose_value,device_tm_bin)
...
###########################################################
# ADDING MISSING device_tm_bin cells + NAs AND IMPUTATION #
###########################################################

length.IDs <- length(IDs)
length.visit <- 8 #by default we expect to see 8 days of measurements in a single visit
length.obsns <- length.visit*288 #the number of measurements we expect to see in a single visit, assuming no missing values
...
for (i in 2:length.IDs){
  ...
  #get function2 from patient with pt_id = IDs[i]
  function2 <- ds_cgm_sub1 %>%
    filter(pt_id == IDs[i], as.character(visitnum) == as.character(thisvisit)) %>%
    select(pt_id,trt_group,visitnum,device_visit_period_day,glucose_value,device_tm_bin)
...
#plot to check behavior/see how many NA values there are on the first and last days of a visit
ex %>%
  filter(device_visit_period_day == 1) %>%
  ggplot(aes(x = device_tm_bin, y = glucose_value, color = pt_id)) + 
  geom_line() +
  ggtitle("visit 5, day 1: checking for missing values at beginning of measurement period")
ggsave("04_missing_val_check_v5d1",device="png")

ex %>%
  filter(device_visit_period_day == 8) %>%
  ggplot(aes(x = device_tm_bin, y = glucose_value, color = pt_id)) + 
  geom_line() +
  ggtitle("visit 5, day 8: checking for missing values at end of measurement period")
ggsave("04_missing_val_check_v5d8",device="png")
...
#create new column to store imputed values
ex$glucose_imputed <- rep(0,288*length.IDs*length.visit)
...
#store baseline reference glucose data (this is unchanged from the original glucose data)
ex$glucose_aligned[1:288] <- f1.imputed$glucose_imputed
...
ex %>%
  ggplot(aes(x = device_tm_bin, y = glucose_imputed, color = factor(pt_id))) + 
  geom_line() +
  ggtitle("visit 5, day 2 curve data after imputation and prior to srvf alignment")
ggsave("04_ex_curves_before_srvf_alignment.png",device="png")

ex %>%
  ggplot(aes(x = device_tm_bin, y = glucose_aligned, color = factor(pt_id))) + 
  geom_line() +
  ggtitle("visit 5, day 2 curve data after imputation and srvf alignment")
ggsave("04_ex_curves_after_srvf_alignment.png",device="png")
...
```{r eda}
#checking degree of missing data at beginning of measurement time periods
ex %>%
  filter() %>%
  ggplot(aes(x = device_tm_bin, y = glucose_value, color = factor(pt_id))) + 
  geom_line()
...
```{r examplefdasrvf}
#nothing special here. just understanding how fdasrvf::pair_align_functions requires arguments to be formatted
data("simu_data")
