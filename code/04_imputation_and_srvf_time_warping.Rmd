...
#add in missing values/rows via complete (allows for preservation of trt_group in resulting df); called resulting new dataframe "ex"
...
ex %>%
  ...
  geom_line() +
  ggtitle("At randomization: prior to alignment of 10 patients to reference patient curve") + 
  scale_x_datetime(date_labels = "%H:%M", date_breaks = "2 hours") +
...
ex %>%
  ...
  ggplot(aes(x = as.POSIXct(device_tm_bin_start_tm), y = glucose_aligned, color = pt_id, group = pt_id)) + 
  geom_line() +
  ggtitle("At randomization: after alignment of 10 patients to reference patient curve") + 
  scale_x_datetime(date_labels = "%H:%M", date_breaks = "2 hours") +
  xlab("time of day (hr:min)") +
  ylab("glucose values")
...
out10 %>% ggplot(aes(x = device_tm_bin/288, y = warpfxn, color = pt_id, group = pt_id)) + 
  geom_line() +
  ggtitle("At randomization: warp functions for 10 patients after alignment to ref. patient curve") + 
  #scale_x_datetime(date_labels = "%H:%M", date_breaks = "2 hours") +
