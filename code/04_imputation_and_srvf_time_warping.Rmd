...
#order ds_cgm_sub10 by patient ID, date, then time
#ds_cgm_sub10.ordered <- ds_cgm_sub10[with(ds_cgm_sub10,order(pt_id,device_dt_tm_days_from_enroll,device_tm)),]
#ds_cgm_sub10.ordered$device_tm <- as.numeric(ds_cgm_sub10.ordered$device_tm)

#bin time values in 5-minute increments
#ds_cgm_sub10.ordered$device_tm_bin = cut(ds_cgm_sub10.ordered$device_tm, #breaks=seq(0,86400,by=300),include.lowest=TRUE,labels=seq(1,288,by=1))
#ds_cgm_sub10.ordered$device_tm_bin <- as.numeric(ds_cgm_sub10.ordered$device_tm_bin)
...
# ds_cgm_sub10_pt_first_day <- ds_cgm_sub10.ordered %>%
#   group_by(pt_id) %>%
...
# ds_cgm_sub10_plus_day <- ds_cgm_sub10.ordered %>%
#   left_join(ds_cgm_sub10_pt_first_day, by = "pt_id") %>%
...
#get all patient IDs
IDs <- unique(ds_cgm_sub10$pt_id,incomparables = FALSE)

#identify baseline patient ID
base_id <- IDs[1]

#get all visit periods
visits <- ds_cgm_sub10$visitnum
...
function1 <- ds_cgm_sub10 %>%
  filter(pt_id == base_id, as.character(visitnum) == as.character(thisvisit), day_in_subrange %in% 1:3) %>%
...
length.days <- 3 #corresponds to number of consecutive days over which patient data was selected
...
for (i in 2:length.IDs){
  ...
  function2 <- ds_cgm_sub10 %>%
    filter(pt_id == IDs[i], as.character(visitnum) == as.character(thisvisit), day_in_subrange %in% 1:3) %>%
    select(pt_id,trt_group,visitnum,day_in_subrange,glucose_value,device_tm_bin)
...
#add in missing values/rows via spread/gather: option 1
# ex <- current.df %>%
#   mutate(pt_id = factor(pt_id,levels=unique(pt_id))) %>% #preserve original ordering of obsns by pt_id
#   spread(pt_id, glucose_value, fill = NA, drop=TRUE) %>% 
#   gather(pt_id, glucose_value, -(1:4)) #gather excluding the visitnum,device_visit_period_day,device_tm_bin columns in spread representation


#add in missing values/rows via spread/gather: option 2
# ex <- current.df %>%
#   mutate(pt_id = factor(pt_id,levels=unique(pt_id))) %>% #preserve original ordering of obsns by pt_id
...
for (i in 1:length.IDs){
  ...
  first <- 1+(i-1)*length.obsns*length.days
  last <- i*length.obsns*length.days
