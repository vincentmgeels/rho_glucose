...
1. Identify an initial patient via pt_id. WLOG take pt_id = 1 and define pt_id 1 = BASELINE_PT_ID.
2. Extract patient BASELINE_PT_ID's paired glucose/time measurements for visit 0 and day i according to "reasonable" glucose level change patterns (based on visual analysis). WLOG take i = 1.
...
4. Set function1 = patient PRIMARY_PT_ID's matrix of paired glucose/time measurements on visit 0, day 1. All other patient paired glucose/time measurements at the day level will now be aligned to function1 in the next step.
5. for (i in (all patient IDs)) do
	...
	for (j in 0:length(visits)) do
		for (k in 1:length(days)) do
			...
			set function2 = patient NEW_PT_ID's matrix of paired glucose/time measurements
...
# ds_cgm_complete <- readRDS("C:/Users/vince/Documents/Life/IMSM/Rho_glucose/ds_cgm.RDS")
ds_cgm_complete <- readRDS("../data/ds_cgm.RDS")

#our updated dataset containing consecutive 3-day intervals from all visit periods
...
function1 <- ds_cgm_complete_3days %>%
  filter(pt_id == IDs[57], as.character(visitnum) == as.character(0), day_in_subrange %in% 1:3) %>%
...
pt10_1 <- ex %>%
  filter(pt_id %in% c(IDs[57:66]),visitnum == 0, day_in_subrange == 1) %>%
  ggplot(aes(x = as.POSIXct(device_tm_bin_start_tm), y = glucose_imputed, color = pt_id)) + 
  geom_line() +
  ggtitle("10 patients prior to alignment") + 
...
pt10_2 <- ex %>%
  ...
  ggplot(aes(x = as.POSIXct(device_tm_bin_start_tm), y = glucose_aligned, color = pt_id, group = pt_id)) + 
  geom_line() +
  ggtitle("10 patients after alignment") + 
  theme(legend.position = "none") +
  scale_x_datetime(date_labels = "%H:%M", date_breaks = "2 hours") +
  xlab("system time (hr:min)") +
  ylab("glucose values")
#plot pt10_2 solo
#ggsave("04_10pt_ex_curves_after_imputation_and_alignment.png",device="png")

#plot pt10_1 and pt10_2 together
#ggsave("04_10pt_plots_stacked.png",arrangeGrob(pt10_1,pt10_2),device="png", width=7,height=10)


#plot warp functions (vs. time) used to do the srvf alignment for the 10 original patient curves
pt10_3 <- out10 %>% ggplot(aes(x = device_tm_bin/288, y = warpfxn, color = pt_id)) + geom_line() +
  ggtitle("Warp paths for 10 patients after alignment") + 
  theme(legend.position = "none") +
...
#plot pt10_1-3 together
grid.arrange(pt10_1,pt10_2,pt10_3, ncol=1)
  

# example plots for linear imputation step
pt_nolinim <- ex %>%
  filter(pt_id %in% c(IDs[33]),visitnum == 1, day_in_subrange == 1) %>%
  ggplot(aes(x = as.POSIXct(device_tm_bin_start_tm), y = glucose_value, color = pt_id, group = pt_id)) + 
  geom_line() +
  ggtitle("example: patient curve prior to linear imputation") + 
  theme(legend.position = "none") +
  scale_x_datetime(date_labels = "%H:%M", date_breaks = "2 hours") +
  xlab("time of day (hr:min)") +
  ylab("glucose values")
...
pt_linim <- ex %>%
  ...
  geom_line() +
  ggtitle("example: patient curve after linear imputation") + 
  theme(legend.position = "none") +
  scale_x_datetime(date_labels = "%H:%M", date_breaks = "2 hours") +
  xlab("time of day (hr:min)") +
  ylab("glucose values")
...
grid.arrange(pt_nolinim,pt_linim, ncol=1)
...
#get NAs in dataset we worked with
numNAs <- apply(is.na(ex), 2, sum)
#get total number of observations in dataset we worked with
length(ex$pt_id)
#16313 total NA values out of 1057536 total observations in the subset we worked with
#-> 1.5% of observations are missing
